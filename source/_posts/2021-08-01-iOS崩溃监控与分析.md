...
```bash
atos [-arch 架构名] [-o 符号表] [-l 模块地址] [方法地址]  # 模块地址、方法地址都是运行时地址（MachO中地址+ ASLR Offset）
# 比如
atos -o xxx.app.dSYM -arch arm64/armv7 -l loadAddress runtimeAddress # 模块地址可参考6.1中计算得出
```
