...
Jenkins 的安装一般都会专门使用一台 Mac 作为打包机（一般是 Mac Mini(主机盒子)，大厂会有专门的打包集群）。

> 不建议使用黑苹果，有可能出现Android没问题、iOS打包失败等一些奇怪的问题。

如果所在公司可能没有专门的打包机，感觉就没有必要在自己的机器上再安装 jenkins 了，直接在终端执行脚本就可以了。步骤如下：
